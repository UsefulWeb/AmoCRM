...
1. [Множественные операции](#множественные-операции)
    1. [find](#crmleadfind)
    2. [findById](#crmleadfindbyid)
    3. [insert](#crmleadinsert)
    4. [update](#crmleadupdate)
    5. [from](#crmleadfrom)
    6. [new crm.Lead](#создание)
    7. [of](#создание)
    8. [create](#создание)
2. [Объект сделки](#работа-с-объектом-сделки)
    1. [save](#save)
    2. [fetch](#fetch)
    3. [exists](#exists)
3. [Контакты сделки](#работа-с-контактами-сделки)
    1. [linkContacts](#linkcontacts)
    2. [getContacts](#getcontacts)
    3. [unlinkContacts](#unlinkcontacts)
4. [Компания сделки](#работа-с-компанией-сделки)
    1. [linkCompany](#linkcompany)
    2. [getCompany](#getcompany)
    3. [unlinkCompany](#unlinkcompany)
5. [Примечания сделки](#работа-с-примечаниями-сделки)
    1. [new Note](#new-note)
    2. [getNotes](#getnotes)
    3. [addNotes](#addnotes)
6. [Задачи сделки](#работа-с-задачами-сделки)
    1. [new Task](#new-task)
    2. [getTasks](#gettasks)
    3. [addTasks](#addtasks)
7. [Дополнительные поля сделок](#работа-с-дополнительными-полями-сделок)
    1. [new crm.Lead.Field](#new-crmleadfield)
...
|[create](#создание)         |Аналогично new crm.Lead                                |
|[of](#создание)             |Создаёт множество объектов сделки                      |
...
| Метод                                         | Описание                                      |
|-----------------------------------------------|-----------------------------------------------|
|[linkCompany](#linkcompany)      |Прикрепляет компанию к сделке                  |
|[getCompany](#getcompany)         |Получает компанию, которая прикреплена к сделке|
|[unlinkCompany](#unlinkcompany)|Открепляет компанию от сделки                  |

### Работа с примечаниями сделки

| Метод                            | Описание                                          |
|----------------------------------|---------------------------------------------------|
|[new Note](#new-note)|Создаёт заметку, которая будет прикреплена к сделке|
|[getNotes](#getnotes)  |Получает список примечаний для данной сделки       |
|[addNotes](#addnotes)  |Прикрепляет к сделке массив примечаний             |

### Работа с задачами сделки

| Метод                            | Описание                                         |
|----------------------------------|--------------------------------------------------|
|[new Task](#new-task)|Создаёт задачу, которая будет прикреплена к сделке|
|[getTasks](#gettasks)  |Получает все задачи, прикреплённые к сделке       |
|[addTasks](#addtasks)  |Прикрепляет к сделке массив примечаний            |

### Работа с дополнительными полями сделок

| Метод                                                      | Описание                                     |
|------------------------------------------------------------|----------------------------------------------|
|[new crm.Lead.Field](#new-crmleadfield)|Создаёт дополнительное поле у сделки          |
|[crm.Lead.getFields](#crmleadgetfields)  |Получает все произвольные поля сделок         |
|[crm.Lead.addFields](#crmleadaddfields)  |Прикрепляет к сделке массив произвольных полей|
...
Аналогичую работу выполняет *crm.Lead.create*
...
Для создания множества сделок с начальными атрибутами
можно воспользоваться *crm.Lead.of*

```js
const leads = crm.Lead.of([
  {
    name: 'Покупка карандашей',
    responsible_user_id: '957083',
    sale: '5000'
  ...
  {
    name: 'Покупка ластиков',
    responsible_user_id: '450332',
    sale: '10000'
  }
]);
```

Данные сделки не будут добавлены в CRM. Для их добавления
потребуется вызвать метод *save()* у каждой. Если вы хотите
сразу добавить или обновить сделки, рекомендуем воспользоваться
*crm.Lead.insert* или *crm.Lead.update*.
